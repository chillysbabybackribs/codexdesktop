...
| 6. Neutral tool registry + MCP facade | **DONE (this session)** | `src/main/tools/browser-tool-registry.ts`: the single tool dispatch (moved from dynamic-tool-router, which is now a thin Codex adapter mapping `item/tool/call` onto it). Owner-optional design: Codex passes turn ownership (queueing/blocking); external transports pass null (direct execution, the socket's historical semantics). Socket gained generic `POST /tool/<name>` over the same dispatch. `src/main/workers/mcp-browser-shim.ts`: standalone MCP stdio server (own 3.8 kB bundle, hand-rolled JSON-RPC, zero new deps) serving the canonical specs via the shim-safe `browser-tool-specs.ts` and proxying tools/call to the socket — Claude CLI spawns it with plain node + `CODEX_BROWSER_SOCK`. Verified: 356/356 tests (router tests pass through the registry), MCP protocol round-trip smoke (initialize / tools-list from canonical specs / tools-call with text+image content), and live: `/tool/browser_navigate` drove the real browser, and the shim returned real page content from a running app. |
| 7. Claude process-lifecycle policy | **DONE — APPROVED** | `claude-prep-step7-process-policy-2026-07-19.md`: bounded per-session processes; cap 3 live with visible queueing; 15-min idle-kill (never mid-turn) + tested resume; exact-pinned npm runtime, self-update off, startup version gate; SDK-vs-CLI resolved by spike. |
