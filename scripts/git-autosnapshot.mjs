...
const pushBranch = process.env.CODEXDESKTOP_AUTOGIT_TARGET_BRANCH?.trim() || 'master';
...
async function autoSnapshot(repo) {
  try {
    ...
    try {
      ...
      if (headSha) {
        await requireGit(
          runGit(repo.repoRoot, ['read-tree', headSha], { env: indexEnv }),
          'Preparing autosnapshot index',
        );
      }
      ...
      const treeSha = (
        await requireGit(
          runGit(repo.repoRoot, ['write-tree'], { env: indexEnv }),
          'Writing autosnapshot tree',
        )
      ).trim();
      ...
      if (pushEnabled) {
        await pushSnapshot(repo, branch, pushBranch);
      }
...
async function discoverRepo(cwd) {
  ...
  const repoRootRaw = await requireGit(
    runGit(cwd, ['rev-parse', '--show-toplevel']),
    'Finding git root',
  );
  const repoRoot = await fs.realpath(repoRootRaw.trim());
  const gitDirRaw = (
    await requireGit(runGit(repoRoot, ['rev-parse', '--git-dir']), 'Finding git dir')
  ).trim();
...
async function readTreeSha(repoRoot, headSha) {
  return (
    await requireGit(runGit(repoRoot, ['rev-parse', `${headSha}^{tree}`]), 'Reading git tree')
  ).trim();
}
...
async function createCommit(repo, treeSha, headSha, identityEnv, prepared) {
  ...
  return (
    await requireGit(
      runGit(repo.repoRoot, args, { env: identityEnv }),
      'Creating autosnapshot commit',
    )
  ).trim();
}
...
async function pushSnapshot(repo, sourceBranch, targetBranch) {
  if (!sourceBranch || sourceBranch.startsWith('detached@')) {
    throw new Error('cannot push an autosnapshot from a detached HEAD');
  }

  await requireGit(
    runGit(repo.repoRoot, ['push', 'origin', `HEAD:${targetBranch}`]),
    `Pushing autosnapshot from ${sourceBranch} to origin/${targetBranch}`,
  );
}
...
async function gitAddPaths(repoRoot, paths, env) {
  for (const chunk of chunks(paths, pathChunkSize)) {
    await requireGit(
      runGit(repoRoot, ['add', '-A', '--', ...chunk], { env }),
      'Staging autosnapshot paths',
    );
  }
}
...
async function gitResetIndexPaths(repoRoot, paths) {
  for (const chunk of chunks(paths, pathChunkSize)) {
    await requireGit(
      runGit(repoRoot, ['reset', '-q', 'HEAD', '--', ...chunk]),
      'Refreshing git index',
    );
  }
}
