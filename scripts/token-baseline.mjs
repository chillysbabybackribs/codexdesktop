...
function summarizeScenario(kind, thread, turns) {
  const calls = turns.flatMap((turn) =>
    dedupeCalls(turn.tokenEvents).map((event) => ({
      turn: turn.index,
  ...
  return {
    ...
    finalInputGrowthFromFirst:
      firstCallInput === null || !input.length ? null : input.at(-1) - firstCallInput,
    maximumInputGrowthFromFirst:
      firstCallInput === null || !input.length ? null : Math.max(...input) - firstCallInput,
    cumulativeInput,
...
function buildReport(scenarios) {
  ...
  return {
    ...
    comparison: {
      ...
      finalInputGrowthDelta: nullableDelta(
        tool.finalInputGrowthFromFirst,
        noTool.finalInputGrowthFromFirst,
      ),
      maximumInputGrowthDelta: nullableDelta(
        tool.maximumInputGrowthFromFirst,
        noTool.maximumInputGrowthFromFirst,
      ),
...
class AppServerHarness {
  ...
  async start() {
    ...
    await this.request('initialize', {
      clientInfo: {
        name: 'codexdesktop-token-baseline',
        title: 'Codex Desktop Token Baseline',
        version: '1.0.0',
      },
  ...
  write(message) {
    if (!this.child?.stdin.writable)
      throw new Error(`app-server stdin is unavailable: ${this.stderr.join('\n')}`);
...
function parseArgs(args) {
  ...
  if (
    values.order.length !== 2 ||
    !values.order.includes('no-tool') ||
    !values.order.includes('tool')
  ) {
