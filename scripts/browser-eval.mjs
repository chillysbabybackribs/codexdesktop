...
const LEGACY_REFERENCE = {
  ...
  prompt:
    'ok navigate to reddit and check the last 3 notifications read or unread and monitor this for a speed test',
  wallMs: 40_419,
...
const SCENARIOS = {
  'reddit-notifications': {
    ...
    canonicalPrompt:
      'Using the already-open Reddit notifications page, tell me the latest 3 notifications and whether each is read or unread. Do not click anything or change notification state.',
  ...
  'hacker-news-top-stories': {
    ...
    canonicalPrompt:
      'Using the already-open Hacker News front page, give me the top 10 story titles, their points, and links in a compact numbered list.',
    snapshotObjective:
      'Return the top 10 Hacker News front-page stories in ranking order, including each story title, points, and direct story link.',
    expectedCount: 10,
...
async function main() {
  ...
  const hostUnchanged =
    sameOracle(oracleBefore, oracleAfter, scenario) && sameTabs(initialTabs, finalTabs);
  if (!options.skipModel) {
  ...
  const report = {
    ...
    productionControllerExercised:
      direct.productionControllerExercised &&
    ...
    comparison: {
      ...(model.ok && typeof model.wallMs === 'number'
        ? {
            observedEndToEndRatioVsLegacyReference: round(
              LEGACY_REFERENCE.wallMs / model.wallMs,
              2,
            ),
          }
        : {}),
      ...
      caveat:
        'The legacy trace requested navigation and carried a much larger persisted context; the current model arm starts fresh on the already-open page. The ratio is observational, not an isolated causal speedup.',
      qualityPreserved:
        (!scenario.requiresConclusiveSnapshot || direct.exactSnapshotCount === options.samples) &&
        (options.skipModel || model.accuracy?.exactResponse === true) &&
        hostUnchanged,
    },
  };
...
async function runDirectArm(context, oracle, sampleCount) {
  ...
  for (let index = 1; index <= sampleCount; index += 1) {
    ...
    try {
      ...
      samples.push({
        index,
        ok: false,
        wallMs: round(performance.now() - startedAt, 3),
    ...
    samples.push({
      ...
      outputChars:
        finiteNumber(response.resultChars) ?? JSON.stringify(response.result ?? null).length,
      ...
      ...(ok
        ? {}
        : { error: response.error || 'snapshot response did not match the explicit scenario tab' }),
    });
  ...
  return {
    ...
    transports: Object.fromEntries(
      [...new Set(samples.map(({ transport }) => transport).filter(Boolean))].map((transport) => [
        transport,
    ...
    metrics: {
      ...
      returnedDurationMs: summarizeNumbers(
        successful.map(({ returnedDurationMs }) => returnedDurationMs),
      ),
      snapshotDurationMs: summarizeNumbers(
        successful.map(({ snapshotDurationMs }) => snapshotDurationMs),
      ),
...
async function runModelArm(context, oracle, options) {
  ...
  try {
    ...
    const fastPathContract =
      toolCalls.length === 1 &&
      snapshotCalls.length === 1 &&
      snapshotCalls[0].ok === true &&
      completedToolItems.length === 1 &&
      completedToolItems[0].type === 'dynamicToolCall';
    const productionControllerExercised =
      snapshotCalls.length === 1 && snapshotCalls[0].transport === 'browser-control:/snapshot';

    return {
      ok:
        completed.status === 'completed' &&
        accuracy.exactResponse &&
      ...
      tokens: lastUsage
        ? {
            ...
            calls: tokenEvents.map(({ last, modelContextWindow }) => ({
              ...last,
              modelContextWindow,
            })),
          }
...
class AppServerHarness {
  ...
  async start() {
    ...
    await this.request('initialize', {
      clientInfo: {
        name: 'codexdesktop-browser-eval',
        title: 'Codex Desktop Browser Eval',
        version: '1.0.0',
      },
      capabilities: {
  ...
  write(message) {
    if (!this.child?.stdin.writable)
      throw new Error(`benchmark app-server stdin unavailable: ${this.stderr.join('\n')}`);
...
async function routeModelBrowserTool(params, context) {
  if (params.namespace !== null)
    return failureToolResponse(`Unsupported dynamic tool namespace: ${params.namespace}`);
  ...
  if (params.tool === 'browser_extract_page') {
    if (args.frame && args.frame !== 'main')
      return failureToolResponse('The read-only eval supports only the main frame.');
    ...
    return hostToolResponse(
      await postEval(context, program, {
        timeoutMs: clampNumber(args.timeoutMs, 15_000, 250, 60_000),
  ...
  if (params.tool === 'browser_flow') {
    ...
    if (steps.some((step) => !['wait', 'find'].includes(readString(asRecord(step).type)))) {
      return failureToolResponse(
        'The read-only eval allows only browser_flow wait and find steps.',
      );
    }
  ...
  if (params.tool === 'browser_cdp') {
    ...
    return hostToolResponse(
      await socketJson(context.socketPath, 'POST', '/cdp', {
        ...args,
        tab: context.tabId,
      }),
    );
  ...
  return failureToolResponse(
    `${params.tool} is disabled in this read-only benchmark; use browser_snapshot.`,
  );
}
...
function validateReadOnlyProgram(code) {
  const forbidden = [
    [
      /\.\s*(?:click|submit|requestSubmit|dispatchEvent|setAttribute|removeAttribute|toggleAttribute)\s*\(/i,
      'event or attribute mutation',
    ],
    [
      /\.\s*(?:append|appendChild|prepend|remove|removeChild|replaceWith|replaceChildren|insertBefore|insertAdjacent\w*)\s*\(/i,
      'DOM mutation',
    ],
    [
      /\b(?:window\.)?open\s*\(|\blocation\s*(?:=|\.(?:assign|replace|reload)\s*\()|\bhistory\.(?:pushState|replaceState|go|back|forward)\s*\(/i,
      'navigation or new-tab action',
    ],
    [
      /\b(?:fetch|XMLHttpRequest|WebSocket|EventSource)\b|navigator\.sendBeacon\s*\(/i,
      'network mutation',
    ],
    [
      /(?:localStorage|sessionStorage)\.(?:setItem|removeItem|clear)\s*\(|document\.cookie\s*=/i,
      'storage mutation',
    ],
    [
      /\.(?:innerHTML|outerHTML|textContent|innerText|value|checked|selected|disabled)\s*=(?!=)/i,
      'property mutation',
    ],
    [
      /\b(?:eval|Function)\s*\(|\b(?:Element|Node|EventTarget|Document)\.prototype\b/i,
      'dynamic or prototype mutation risk',
    ],
  ];
...
function validateReadOnlyCdp(args) {
  ...
  if (
    ['capabilities', 'events', 'snapshot', 'network', 'networkBody', 'performance'].includes(
      operation,
    )
  )
    return null;
  if (operation !== 'command')
    return `Rejected browser_cdp operation ${operation}; this benchmark is read-only.`;
  ...
  return method && allowed.has(method)
    ? null
    : `Rejected browser_cdp method ${method ?? '(missing)'}; this benchmark is read-only.`;
}

async function captureOracle(context) {
  const response = await postEval(context, context.scenario.oracleProgram, {
    timeoutMs: 5_000,
    maxResultChars: 20_000,
  });
  if (!response.ok)
    throw new Error(
      `Could not capture ${context.scenario.pageLabel} oracle: ${response.error ?? 'unknown error'}`,
    );
  const result = asRecord(response.result);
  if (result.url !== context.expectedUrl)
    throw new Error(`${context.scenario.pageLabel} oracle URL changed to ${String(result.url)}`);
...
function evaluateRedditSnapshot(snapshot, oracle) {
  ...
  const matches = oracle.rows.map((row) => {
    ...
    for (let index = 0; index < items.length; index += 1) {
      ...
      const score = identityOverlap(
        row,
        `${items[index].text ?? ''} ${items[index].name ?? ''} ${items[index].href ?? ''}`,
      );
      if (score > bestScore) {
        bestIndex = index;
        bestScore = score;
    ...
    if (bestIndex < 0 || bestScore < 2)
      return { id: row.id, matched: false, expectedRead: row.read, observedRead: null };
    used.add(bestIndex);
  ...
  return {
    matches,
    exactSnapshot: matches.every(({ matched, correctState }) => matched && correctState),
  };
}
...
function evaluateRedditModelResponse(finalResponse, oracle) {
  ...
  const ordered = matches.every(
    (match, index) => index === 0 || match.lineIndex > matches[index - 1].lineIndex,
  );
  return {
...
function validateRedditOracle(oracle) {
  if (!Array.isArray(oracle.rows) || oracle.rows.length !== 3) {
    throw new Error(
      `Expected exactly 3 Reddit notification rows, found ${Array.isArray(oracle.rows) ? oracle.rows.length : 0}`,
    );
  }
}
...
function projectRedditOracle(oracle) {
  return {
    url: oracle.url,
    rows: oracle.rows.map(({ id, unread, read, text, href, datetime }) => ({
      id,
      unread,
      read,
      text,
...
function evaluateHackerNewsModelResponse(finalResponse, oracle) {
  ...
  const matches = oracle.rows.map((row) => {
    const lineIndex = lines.findIndex((line) =>
      line.toLowerCase().includes(row.title.toLowerCase()),
    );
  ...
  return {
    exactResponse:
      matches.every(
        ({ matched, correctPoints, correctLink }) => matched && correctPoints && correctLink,
      ) && ordered,
    ordered,
    matches,
  };
...
function validateHackerNewsOracle(oracle) {
  if (
    ...
    oracle.rows.length !== 10 ||
    oracle.rows.some(({ title, href, points }) => !title || !href || !Number.isFinite(points))
  ) {
...
function projectHackerNewsOracle(oracle) {
  return {
    url: oracle.url,
    rows: oracle.rows.map(({ rank, title, href, points }) => ({ rank, title, href, points })),
  };
}
...
async function postEval(context, code, options) {
  ...
  const response = await socketJson(
    context.socketPath,
    'POST',
    `/eval?${query}`,
    code,
...
async function postSnapshot(context, args) {
  ...
  try {
    ...
    if (!context.allowEvalFallback) {
      throw new Error(
        `Production browser-control /snapshot is unavailable (${errorMessage(error)}). Use --allow-eval-fallback only for an explicitly labelled compatibility run.`,
      );
    }
    const program = buildPageSnapshotProgram({
      ...
      (response) => {
        ...
        response.on('end', () => {
          ...
          try {
            ...
            reject(
              new Error(
                `Browser socket returned non-JSON (${response.statusCode}): ${text.slice(0, 300)}`,
              ),
            );
            return;
          }
          if ((response.statusCode ?? 500) >= 400)
            reject(new Error(parsed.error ?? `Browser socket HTTP ${response.statusCode}`));
          else resolveRequest(parsed);
        });
      },
    );
    request.setTimeout(65_000, () =>
      request.destroy(new Error('Browser socket request timed out')),
    );
...
function requireSocketPath() {
  ...
  if (!value)
    throw new Error(
      'CODEX_BROWSER_SOCK is required; run this benchmark from the protected Codex Desktop host session.',
    );
  return value;
}

function requireTabs(response) {
  if (response.ok !== true || !Array.isArray(response.tabs))
    throw new Error(`Could not list browser tabs: ${response.error ?? 'invalid response'}`);
...
function requireActiveScenarioTab(tabs, scenario) {
  ...
  if (active.length !== 1 || !scenario.matchesUrl(active[0].url)) {
    throw new Error(
      `The single active visible tab must already be the ${scenario.pageLabel}; the harness will not navigate or activate tabs.`,
    );
  }
  return active[0];
}
...
function validateExplicitTab(value, expected) {
  ...
  return value === expected
    ? null
    : `Rejected tab ${String(value)}; benchmark is pinned to existing tab ${expected}.`;
}
...
function validateSamePage(value, expected) {
  ...
  try {
    ...
    return requested.href === current.href
      ? null
      : `Rejected navigation to ${requested.href}; benchmark is pinned to the already-open scenario page.`;
  } catch {
...
function sameOracle(left, right, scenario) {
  return (
    JSON.stringify(scenario.projectOracle(left)) === JSON.stringify(scenario.projectOracle(right))
  );
}
...
function sameTabs(left, right) {
  const project = (tabs) =>
    tabs
      .map(({ id, url, active }) => ({ id, url, active }))
...
function hostToolResponse(hostResult) {
  return {
    success: hostResult?.ok === true,
    hostResult,
    ...(hostResult?.ok === true ? {} : { error: hostResult?.error ?? 'browser host call failed' }),
  };
}
