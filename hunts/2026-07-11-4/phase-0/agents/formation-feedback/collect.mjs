...
function addRecord(hit, window, tag, artifactRel, storyId = null) {
  ...
  records.push({
    ...
    classificationReason: 'Regex-derived actor and statement lead; requires parent quote audit before evidence-floor credit.',
    moneyType: price ? 'currency_mention' : 'none',
...
const metrics = {
  ...
  heuristicFirsthandLeads: records.filter(r=>r.firsthand).length,
  qualifiedFirsthand: records.filter(r=>r.firsthand && ['medium','high'].includes(r.classificationConfidence)).length,
  currencyMentions: records.filter(r=>r.priceOrWage).length,
  qualifiedExactJobMoney: records.filter(r=>['exact_job_spend','exact_job_wage'].includes(r.moneyType) && ['medium','high'].includes(r.classificationConfidence)).length,
