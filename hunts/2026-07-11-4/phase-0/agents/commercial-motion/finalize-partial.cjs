...
const metrics = {
  ...
  counts: {records: records.length, retrievedVerified: verification.verified || 0, trustmrrApiRows: trust.length, externalEnrichmentRecords: records.length - trust.length, queryEvents: readJsonl(queriesPath).length, failures: readJsonl(failuresPath).length, categories: Object.keys(categories).length, commercialMetricRecords: records.filter(r => r.priceOrWage || r.commercialMetric?.revenueLast30DaysRaw != null || r.commercialMetric?.mrr != null).length, qualifiedExactJobMoney: 0},
