...
function ThoughtBlock({
  ...
  const durationMs =
    !streaming && meta?.startedAtMs && meta?.completedAtMs
...
function cdpScreenshotArtifact(item: DynamicToolCallItem): CdpScreenshotArtifact | null {
  if (
    item.tool !== 'browser_cdp' &&
    item.tool !== 'browser_screenshot' &&
    item.tool !== 'app_screenshot'
  )
    return null;
  ...
  for (const content of item.contentItems ?? []) {
    ...
    try {
      const parsed = JSON.parse(content.text) as {
        result?: unknown;
        screenshot?: Partial<CdpScreenshotArtifact>;
      };
      const payload =
        parsed.result && typeof parsed.result === 'object'
          ? (parsed.result as { screenshot?: Partial<CdpScreenshotArtifact> })
          : parsed;
      ...
      if (
        ...
        continue;
      if (typeof screenshot.mediaType !== 'string' || typeof screenshot.bytes !== 'number')
        continue;
      return {
...
export function currentActionLabel(
  ...
  for (let i = scan.length - 1; i >= 0; i -= 1) {
    ...
    switch (item.type) {
      ...
      case 'dynamicToolCall': {
        const progress =
          item.tool === 'research_web' ? latestItemProgress(itemMeta[item.id]) : null;
