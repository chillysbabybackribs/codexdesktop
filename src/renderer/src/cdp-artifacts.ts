...
export type CdpFileArtifact = {
  ...
  fileName: string
  mediaType: string
  kind: 'pdf' | 'trace' | 'snapshot' | 'response-body' | 'network-stream'
  bytes: number
}

const cdpFileArtifactKinds: ReadonlySet<string> = new Set(['pdf', 'trace', 'snapshot', 'response-body', 'network-stream'])
...
export function cdpFileArtifact(item: DynamicToolCallItem): CdpFileArtifact | null {
  if (item.tool !== 'browser_cdp' && item.tool !== 'browser_network') return null
