...
export function buildPlanBriefing(input: {
  ...
  return [
    ...
    '',
    'Otherwise, before writing the plan, spend a few quick tool calls verifying the load-bearing premises in the live workspace — the current branch, and whether the files, features, or claims the plan depends on actually exist as you believe. Do not trust internal docs or memory for repository facts; they go stale. Verify only — do not begin executing the task itself.',
    'Prefix any premise you could not verify with "ASSUMPTION:" so the doer checks it before relying on it.',
    '',
    'Then write the working plan the doer will execute, folding in any refinement from the reply. Concise and concrete:',
