...
function completedWorkScore(item: string): number {
  ...
  if (/^Tool completed: (browser_cdp|browser_flow|browser_run|browser_network|browser_snapshot|browser_extract_page|browser_live_search|browser_research_dual|research_web|ui_review)$/i.test(item)) return 4
