...
const taskShapingGuidance = [
  ...
  '- When calling `app_screenshot` or `browser_screenshot` from `functions.exec`, its result is a string containing JSON metadata followed by a `data:image/...` URI. Never pass the whole result to `image()`: extract the URI first (for example, `const imageUrl = result.match(/data:image\\/[^\\s]+/)?.[0]; if (imageUrl) image(imageUrl)`). Passing the combined string produces an invalid `image_url` request.',
