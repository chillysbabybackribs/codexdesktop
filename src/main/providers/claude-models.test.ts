...
test('Claude model ids are provider-qualified and reversible', () => {
  ...
  assert.equal(isClaudeModelId('claude-opus-4-8'), true, 'legacy explicit ids keep routing to Claude')
})
...
test('SDK model metadata maps into shared picker rows', () => {
  ...
  assert.deepEqual(catalog[1].supportedReasoningEfforts.map((entry) => entry.reasoningEffort), ['low', 'high', 'max'])
