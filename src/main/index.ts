...
} from './browser/browser-control-server.js';
...
const instanceRole =
  process.env.CODEX_DESKTOP_INSTANCE_ROLE === 'verification' ? 'verification' : 'host';
...
const attachmentStore = new AttachmentStore(() =>
  join(app.getPath('userData'), 'chat-attachments'),
);
...
const browserStateStore = new BrowserStateStore(() =>
  join(app.getPath('userData'), 'browser-state.json'),
);
...
function sendToMainRenderer(channel: string, payload: unknown): void {
  ...
  try {
    ...
    if (
      !/render frame was disposed|webcontents.*destroyed|webcontents.*disposed/i.test(String(error))
    ) {
...
function createWindow(): void {
  ...
  tabManager.onVisit({
    recordVisit: (url, title, favicon) =>
      browserHistoryStore.recordVisit(url, title, Date.now(), favicon),
      ...
      switch (command) {
        ...
        case 'zoom-in':
          if (activeTabId)
            tabManager?.zoom(
              activeTabId,
              command === 'zoom-out' ? 'out' : command === 'zoom-in' ? 'in' : 'reset',
            );
          break;
      }
    },
...
function registerIpc(): void {
  ...
  ipcMain.handle(
    ipcChannels.artifactOpenImage,
    async (_event, params: ArtifactReadImageParams): Promise<boolean> => {
  ...
  ipcMain.handle(
    ...
    async (_event, params: AttachmentPreviewParams): Promise<boolean> => {
  ...
  ipcMain.handle(ipcChannels.browserCloseTab, (_event, tabId: string) =>
    tabManager?.closeTab(tabId),
  );
  ipcMain.handle(ipcChannels.browserActivateTab, (_event, tabId: string) =>
  ...
  ipcMain.handle(ipcChannels.browserForward, (_event, tabId: string) =>
  ...
  ipcMain.handle(
    ...
    (_event, tabId: string, direction: 'in' | 'out' | 'reset') =>
      tabManager?.zoom(tabId, direction),
  );
  ipcMain.handle(ipcChannels.browserToggleMute, (_event, tabId: string) =>
    tabManager?.toggleMute(tabId),
  );
  ipcMain.handle(ipcChannels.browserToggleVpn, () => vpnManager.toggle());
  ipcMain.handle(ipcChannels.browserSetBounds, (_event, bounds: BrowserBounds) =>
  ...
  ipcMain.handle(ipcChannels.browserSetOverlayOpen, (_event, open: boolean) =>
  ...
  ipcMain.handle(ipcChannels.browserOmniboxSelect, (_event, index: number) =>
    omniboxPopup?.setSelection(index),
  );
  ipcMain.handle(ipcChannels.browserOmniboxClose, () => omniboxPopup?.hide());
  ipcMain.handle(
    ipcChannels.browserMenuOpen,
    (_event, anchor: BrowserMenuAnchor, items: BrowserMenuItem[]) =>
      browserMenuPopup?.show(anchor, items),
  );
  ipcMain.handle(ipcChannels.browserMenuUpdate, (_event, items: BrowserMenuItem[]) =>
    browserMenuPopup?.update(items),
  );
      ...
      const notification = new Notification({
        ...
        body:
          ...
          (params.status === 'failed'
            ? 'The background turn failed.'
            : 'The background turn completed.'),
        silent: false,
      });
  ...
  ipcMain.handle(
    ipcChannels.transcriptCacheLoad,
    async (_event, threadId: string): Promise<unknown | null> => {
      const snapshots = await transcriptCache.read(threadId);
      return snapshots.at(-1) ?? null;
    },
  );
  ipcMain.handle(
    ipcChannels.transcriptCachePersist,
    (_event, params: TranscriptCachePersistParams) =>
      transcriptCache.replace(params.threadId, [params.snapshot]),
  );
  ipcMain.handle(
    ipcChannels.checkpointList,
    async (_event, threadId: string): Promise<CheckpointSummary[]> => {
  ...
  ipcMain.handle(
    ...
    async (_event, params: CheckpointRevertParams): Promise<void> => {
  ...
  ipcMain.handle(
    ...
    (_event, params: MentionIndexParams): Promise<MentionIndexResult> =>
  ...
  ipcMain.handle(
    ipcChannels.traceSave,
    async (_event, params: TraceSaveParams): Promise<TraceSaveResult> => {
      if (!mainWindow) {
        return { saved: false };
      }
...
function sanitizeTraceFileName(name: string): string {
  const cleaned =
    name.replace(/[^a-z0-9._-]+/gi, '-').replace(/^-+|-+$/g, '') || 'codex-turn-trace';
